function addUpdateButtonAction(base, tid = -1) {
    ...
    if (isValid) {
        if (tid === -1) {
            ...
            fetchAsync('http://localhost:3000/students', 'POST', data)
                .then((response) => {
            ...
            fetchAsync('http://localhost:3000/students/' + tid, 'PUT', data)
                .then((response) => {
...
function deleteButtonAction(tid) {
    ...
    fetchAsync(`http://localhost:3000/students/${tid}`, 'DELETE', null)
